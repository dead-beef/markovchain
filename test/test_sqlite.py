...
class TestMarkovSqlite(TestCase):
    ...
    def testRandomLink(self):
        m = self.Markov()
        values = list(str(x) for x in range(4))
        m.links(('', y) for y in values)
        counter = Counter(m.random_link(('',))[0] for _ in range(10 * len(values)))
        self.assertEqual(len(counter.items()), len(values))
        self.assertTrue(
            all(count and item in values for item, count in counter.items())
        )

    def testRandomLinkFrequency(self):
        m = self.Markov()
        values = list(range(4))
        counts = map(lambda x: x * x, values)
        m.links(chain(*(repeat(('', str(v)), n) for v, n in zip(values, counts))))
        counter = Counter(m.random_link('')[0] for _ in range(20 * len(values)))
