...
class TestMarkovImage(TestCase):
    ...
    def testGenerate(self):
        ...
        data = list(m.generate(2, 2).getdata()) # pylint: disable=no-member
        self.assertEqual(data, [0, 1, 2, 3])

        data = list(m.generate(4, 2).getdata()) # pylint: disable=no-member
        self.assertEqual(data, [0, 1, 2, 3, 0, 1, 2, 3])

    def testGenerateLevels(self):
        scanner = Scanner(lambda x: x)
        ...
        data = list(m.generate(2, 2, levels=1).getdata()) # pylint: disable=no-member
        self.assertEqual(data, [0, 1, 2, 3])

        data = list(m.generate(2, 2, levels=2).getdata()) # pylint: disable=no-member
        self.assertEqual(
            data,
            [
                0, 1, 1, 2,
        ...
        with self.assertRaises(ValueError):
            m.generate(2, 2, levels=3)
        with self.assertRaises(ValueError):
            m.generate(2, 2, levels=0)

        img = Image.new('P', (1, 1))
        img.putpalette(self.palette)
        img.putpixel((0, 0), 1)

        data = list(m.generate(
            2, 1, levels=1,
